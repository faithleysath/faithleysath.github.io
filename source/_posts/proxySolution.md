---
title: 【Proxy】网络代理设置解决方案合集
date: 2023-10-01 09:44:18
tags:
- Tips
- Windows
- Linux
...
本文用于记录一些`命令行`工具的代理设置方法，`UWP应用`的代理设置，`WSL2`的代理设置等等

# UWP应用走代理

> 引用自：https://www.duohuohub.cn/threads/49/#post-101

微软的OneDrive真好用...但偶尔会出现怎么样都无法登陆的情况
...
> 引用自：https://anuoua.github.io/2019/08/11/配置Powershell命令行代理/
...
2. 输入`code $PROFILE`使用`vscode`打开配置文件，如果没有`vscode`，可输入`$PROFILE`查看配置文件地址，使用其他文本编辑器打开。
3. 在配置文件中输入以下代码：
...
不过要注意的都是，上面设置的命令行代码作用的范围是一次性的，也就是关闭cmd窗口后，就失效了，如果需要永久生效的话，是需要配置系统的环境变量的
